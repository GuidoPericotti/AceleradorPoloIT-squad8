...
const MentorRow = ({ mentor, onEdit, onDelete, openDeleteModal }) => {
  ...
  return (
    <tr className="border-b dark:border-neutral-600 dark:hover:bg-neutral-600">
      ...
      <td className="px-3 py-4">
        {!isEmpty && (
          <div className="flex flex-col space-y-2"> {/* Cambiado a columna */}
            <button
              type="button"
              className="inline-flex items-center border-2 border-md border-[#8BACD6] text-[#8BACD6] rounded-md px-1 py-2 text-xs font-medium uppercase leading-normal transition duration-150 ease-in-out hover:bg-[#8BACD6] hover:text-white"
              onClick={() => onEdit(mentor)}
            >
              Editar
              <svg
